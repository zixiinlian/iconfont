...
var SINGLE_SVG_FONT_TEMPLATE = _.template(
  '<svg height="1000" width="1000.0" xmlns="http://www.w3.org/2000/svg"><path d="${path}"/></svg>'
);
...
module.exports = function fontWorker(taskInfo, callback) {
  ...
  workplan.push(async.apply(fstools.mkdir, path.join(taskInfo.tmpDir, 'svg')));


  (function (next) { 

    var glyphs = taskInfo.builderConfig.glyphs;

    glyphs.forEach(function(item){

      workplan.push(async.apply(fs.writeFile, 
        path.join(taskInfo.tmpDir, 'svg', item.css + '.svg'), 
        SINGLE_SVG_FONT_TEMPLATE(item), 
        'utf8'));
    });
